...
describe("token providers", () => {
	...
	it("uses the peer authorization-server endpoint, requests resource/audience, and caches OAuth tokens", async () => {
		...
		const provider = new OAuthClientCredentialsProvider({ clientId: "client-id", clientSecret: "client-secret" }, async (url, init) => {
	...
	it("does not fall back to a legacy key when an OAuth token request fails", async () => {
		const provider = new OAuthClientCredentialsProvider({ clientId: "id", clientSecret: "secret" }, async () => new Response("denied", { status: 401 }));
